...
abstract class StripeBase extends AbstractPaymentProcessor {
  ...
  protected constructor(_, options, container: InjectedDependencies) {
    super(_, options);
  ...
  async updateStripeKeys() {
    const { metadata }: { metadata: any } = await this.storeService_.retrieve();
    this.stripe_ = metadata?.stripe.secretKey;
  }
